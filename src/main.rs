...
use anyhow::Result;
use nn::{ModuleT, OptimizerConfig,Sgd,VarStore};
use nnetwork::*;
use tch::*;
fn grad_example() {
...
//     for epoch in 1..150 {
//         opt.set_lr(learning_rate(epoch));
//         for (i,(bimages, blabels)) in m.train_iter(64).shuffle().to_device(vs.device()).enumerate() {
//             if i%100 == 0 {
//                 let test_accuracy =
//                     net.batch_accuracy_for_logits(&m.test_images, &m.test_labels, vs.device(), 512);
//                 println!("epoch: {:4}, batch: {:5}, test acc: {:5.2}%", epoch,i, 100. * test_accuracy,);
//             }
//             let bimages = tch::vision::dataset::augmentation(&bimages, true, 4, 8);
